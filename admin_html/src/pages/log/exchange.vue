<template>
 ...
 <el-col :xs="24" :sm="12" :lg="4">
     ...
     <el-col :xs="24" :sm="12" :lg="4">
          <el-input placeholder="订单号" v-model="searchParameter.wares_no"></el-input>
       </el-col>
       <el-col :xs="24" :sm="12" :lg="4">
            <el-input placeholder="商品名" v-model="searchParameter.wares_name"></el-input>
         </el-col>
  </el-row>
  <el-row>
  ...
  <el-table height="80vh" style="margin-top: 15px" border :data="roleList" :showPage="false">
      <el-table-column width="60" label="ID" prop="id"/>
      <el-table-column width="100" label=" ID/用户/电话" prop="user_name">
      ...
      <el-table-column label="订单号" prop="wares_no"/>
      <el-table-column label="商品名称" prop="wares_name"/>
      <el-table-column label="商品规格" prop="wares_spec"/>
      <el-table-column label="积分" prop="wares_money"/>
      ...
      <el-table-column fixed="right" label="操作" width="150px">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-edit" @click="showDialog('edit',scope.row)" v-if="scope.row.status<3">
            订单状态编辑
          </el-button>
     ...
     <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="订单状态">
            <el-radio-group v-model="form.status" size="medium">
              <el-radio  :key="0" :label="0">下单</el-radio>
              <el-radio  :key="1" :label="1">发货中</el-radio>
              <el-radio  :key="2" :label="2">运输中</el-radio>
              <el-radio  :key="3" :label="3">签收</el-radio>
            </el-radio-group>
          </el-form-item>
      </el-form>
...
<script>
    import {exchangeListApi,exchangeEditApi} from '@/api/Wares'
  export default {
    data () {
      return {
        searchParameter: {},
   ...
   methods: {
     ...
     getUserList(){
       ...
       exchangeListApi({...this.searchParameter, limit: this.pageSize}).then(res => {
         this.roleList = res.data.data;
          this.allpage = res.data.total;
       })
     ...
     search() {
      ...
      onSubmit() {
        if(this.form.id){
          exchangeEditApi(this.form).then(res => {
            if(res.code == 1){
             this.dvEdit = false
             this.$message({message:res.msg,type: 'success'});
