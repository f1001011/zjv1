...
//        }
    ...
    public function ThighDownload()
    {
        ...
        if (empty($name)) {
            return show([], config('ToConfig.http_code.error'), language(10018));
        }
    ...
    public function UpdatePwd()
    {
        ...
        if (empty($sfz)) {
            return show(0, [], language(10019));
        }
        if (empty($PastPwd)) {
            return show(0, [], language(10020));
        }
        if (empty($pwd)) {
            return show(0, [], language(10021));
        }
        if (empty($upwd)) {
            return show(0, [], language(10022));
        }

        // 验证失败
        if (empty($captcha) || !$this->captcha($captcha)) {
            return show(0, [], language(10005));
        }
        ...
        $UserInfo = UserModel::page_one([['id', '=', $this->UserInfo->id]]);
        if ($sfz != $UserInfo['sfz']) {
            return show(0, [], language(10023));
        }

        $gpwd = $UserInfo->getData('pwd');

        if ($gpwd != md5($PastPwd)) {
            return show(0, [], language(10024));
        }
...
//        $sum = SendModel::toDayMoney($map);
    ...
    public function address()
    {
        ...
        if (empty($UserName)) {
            return show(0, [], language(10026));
        }
        if (empty($phone)) {
            return show(0, [], language(10027));
        }
        if (empty($province)) {
            return show(0, [], language(10028));
        }
        if (empty($city)) {
            return show(0, [], language(10029));
        }
        if (empty($county)) {
            return show(0, [], language(10030));
        }
        if (empty($address)) {
            return show(0, [], language(10031));
        }
        //不能重复添加地址
        $where = array();
        ...
        if ($res) {
            return show(0, language(10032), language(10032));
        }
    ...
    public function DelAddress()
    {
        $aid = $this->request->post('aid/d', 0);
        if ($aid <= 0) {
            return show(0, [], language(10034));
        }
        $find = AddressModel::where(['uid' => $this->UserInfo->id, 'id' => $aid])->find();
        if (empty($find)) {
            return show(0, [], language(10035));
        }
    ...
    public function WithdrawPwd()
    {
        ...
        if (empty($pwd) || empty($sfz) || $pwd < 4) {
            return show(0, [], language(10036));
        }

        if ($pwd == 123456 || strlen($pwd) != 6){
            return show(0, [], language(10037));
        }
        // 验证失败
        if (empty($captcha)) {
            // if (empty($captcha) || !$this->captcha($captcha)) {
    ...
    public function bank()
    {
        ...
        if ($res['user_name'] != $AccountName) {
            return show(0, [], language(10040));
        }
        if (BankModel::where('card', $card)->find()) {
            return show(0, [], language(10041));
        }
        if ($default > 0) {
    ...
    public function DelBank()
    {
        $bid = $this->request->post('bid/d', 0);
        if ($bid <= 0) {
            return show(0, [], language(10044));
        }
        $find = BankModel::where(['u_id' => $this->UserInfo->id, 'id' => $bid])->find();
        if (empty($find)) {
            return show(0, [], language(10045));
        }
    ...
    public function WithdrawalAdd()
    {
        ...
        if ($money <$withdrawal_start ){
            return show(0, [], sprintf(language(10056), $withdrawal_start));
        }
        ...
        if ($UserInfo['is_real_name'] != 2) {
            return show(0, [], language(10047));
        }
        if ($UserInfo->getData('withdraw_pwd') != base64_encode($pwd)) {
            return show(0, [], language(10048));
        }

        if ($UserInfo['money_approve'] < $money) {
            return show(0, [], language(10049));
        }
