...
//        }
    ...
    public function UpdatePwd()
    {
        ...
        if (empty($sfz)) {
            return show(0, [], '身份证不能为空');
        }
        if (empty($PastPwd)) {
            return show(0, [], '旧密码不能为空');
        }
        if (empty($pwd)) {
            return show(0, [], '密码不能为空');
        }
        if (empty($upwd)) {
            return show(0, [], '重复密码不能为空');
        }
...
//        $sum = SendModel::toDayMoney($map);
    ...
    public function address()
    {
        ...
        if (empty($UserName)) {
            return addShow(0, '名称错误', '名称错误');
        }
        if (empty($phone)) {
            return addShow(0, '电话错误', '电话错误');
        }
        if (empty($province)) {
            return addShow(0, '省错误', '省错误');
        }
        if (empty($city)) {
            return addShow(0, '市错误', '市错误');
        }
        if (empty($county)) {
            return addShow(0, '县错误', '县错误');
        }
        if (empty($address)) {
            return addShow(0, '详细地址错误', '详细地址错误');
        }
        //不能重复添加地址
        $where = array();
        ...
        if ($res) {
            return addShow(0,'地址已存在不能重复添加','地址已存在不能重复添加');
        }
    ...
    public function EditAddress()
    {
        ...
        if ($aid <= 0) {
            return addShow(0, '地址ID错误', '地址ID错误');
        }
        //选择默认地址的时候，把其他的都设置为不默认地址
        if ($default > 0) {
            AddressModel::where(['uid' => $this->UserInfo->id])->update(['is_default' => 0]);
    ...
    public function WithdrawPwd()
    {
        ...
        if (empty($pwd) || empty($sfz) || $pwd < 4) {
            return show(0, [], '身份证密码填写错误');
        }

        if ($pwd == 123456 || strlen($pwd) != 6){
            return show(0, [], '密码过于简单');
        }
        // 验证失败
        if (empty($captcha)) {
            // if (empty($captcha) || !$this->captcha($captcha)) {
    ...
    public function bank()
    {
        ...
        if ($res['user_name'] != $AccountName) {
            return show(0, [], '卡用户名和实名不匹配');
        }
        if (BankModel::where('card', $card)->find()) {
            return show(0, [], '卡号已经存在');
        }
        if ($default > 0) {
    ...
    public function WithdrawalAdd()
    {
        ...
        if ($money <$withdrawal_start ){
            return show(0, [], "最少提现{$withdrawal_start}");
        }
...
//        }
    ...
    public function UserSubordinate()
    {
        ...
        //查询我的下级用户有那些
        $array1 = UserModel::where('agent_id_1|agent_id_2|agent_id_3', $this->UserInfo->id)->column('id');
        //查询购买了 产品的用户
        $array2 = GoodsOrderModel::group('uid')->column('uid');
        // 找到两个数组的交集
