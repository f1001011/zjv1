<?php
//config('ToConfig.app_update.image_url')
return [
    //返回code
    'error_code'=>0,//失败code
    'success_code'=>1,//code
    'captcha'=>'aa123456',//万能验证码
    'code'=>'10000',
    'replace_img_prefix' => 'https://www.alufdff.cn/storage/',      //旧域名站点图片替换为新站点图片使用image_url的域名替换，主要用于编辑器内容的图片替换防止碎图
    'image_url'=>'https://www.alufdff.cn/storage/',//图片地址
    'touxiang_url'=>'https://www.eolzpwo.cn/',//头像地址
    'qrcode_url'=>'https://www.eolzpwo.cn/',//二维码地址
    'tg_url'=>'http://www.lszgfreer.top/',//二维码地址
    'sign_merch'=>'2zn7s4m0uctu',//sign 验证的商户号
    //加密公钥
    'public_key'=>'-----BEGIN PUBLIC KEY-----

0wIDAQAB
-----END PUBLIC KEY-----',
    'private_key'=>'-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----'
];